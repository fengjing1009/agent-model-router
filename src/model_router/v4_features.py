...
Phase 3's 390-dim online assembly lives in `model_router.engine.features`.
    ...
    def _ensure_bge(self):
        if self._bge is None:
            if self.backend == "onnx":
                from model_router.bge_onnx import OnnxBGE
...
class V4FeatureExtractor:
    ...
    `model_router.engine.features.build_feature_bundle`. This class remains
    ...
    def build_feature_vector(self, *,
        ...
        import numpy as np
        from model_router.features import (
            ContextMetadata,
